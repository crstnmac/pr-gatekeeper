...
function displayResults(result: AnalysisResult): void {
  ...
  if (policyResults.length > 0) {
    ...
    const failedResults = policyResults.filter(r => r.status === 'failed');
    const warningResults = policyResults.filter(r => r.status === 'warning');

    console.log(`│  Passed: ${passed}  |  Failed: ${failedResults.length}  |  Warnings: ${warningResults.length}`.padEnd(60) + '│');
  ...
  console.log('├─────────────────────────────────────────────────────────────┤');
  console.log(
    `│  ${actionEmoji[decision.action] || ''} ${actionText[decision.action] || ''}`.padEnd(60) + '│'
  );
  console.log(`│  Confidence: ${(decision.confidence * 100).toFixed(1)}%`.padEnd(60) + '│');
  console.log('│                                                               │');
  console.log(`│  ${decision.reasoning.summary.substring(0, 56)}`.padEnd(60) + '│');
  console.log('└─────────────────────────────────────────────────────────────┘');
  console.log();
